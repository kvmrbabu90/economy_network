...
const COLOR_MIXED    = { r: 230, g: 152, b: 40 };   // amber — opposing signals
 ...
 *  Returns null for no_effect (node is dimmed / hidden in impact mode).
 *  mixed_signals nodes use amber regardless of net direction. */
export function tintColor(verdict: ImpactVerdict | undefined): string | null {
  ...
  const base = verdict.mixed_signals
    ? COLOR_MIXED
    : verdict.direction === "positive" ? COLOR_POSITIVE : COLOR_NEGATIVE;
...
/** Hex / rgb helper used by 3D mesh material. Mirrors tintColor logic. */
...
export function buildMultiImpactState(g: EconGraph, resp: MultiImpactResponse): ImpactState {
  const byNode = new Map<string, ImpactVerdict>();
  for (const v of resp.merged) byNode.set(v.node_id, v);
  const chainEdges = new Set<string>();
  g.forEachEdge((eid, _attrs, src, tgt) => {
